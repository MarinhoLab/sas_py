...
using C = marinholab::sas::core::Clock;
using SS = marinholab::sas::core::ShutdownSignaler;
...
collection is enabled.)pbdoc")
            .value("Mean",    marinholab::sas::core::Statistics::Mean)
            .export_values();
...
Callers must wait on `should_shutdown()` to observe the change.)pbdoc");
    ...
    clock.def(py::init<double, bool>(), py::arg("sampling_time_in_seconds"),
              py::arg("enable_statistics") = true,
        R"pbdoc(@brief Construct a Clock.

@param sampling_time_in_seconds Desired sampling time (seconds).
@param enable_statistics Whether to enable internal statistics collection (default: True).)pbdoc");
...
@param statistics The statistic type to query (see `Statistics`).
